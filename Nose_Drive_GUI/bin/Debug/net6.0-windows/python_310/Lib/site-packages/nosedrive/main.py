  ...
  with mp_face_mesh.FaceMesh(
    ...
    while cap.isOpened():      
      ...
      if not calibrated:        
        ...
        # CURSOR ON CENTER      
        #Getting window position, width and height
        x, y, width, height = cv2.getWindowImageRect('Nose Drive Tracking')
        
        #Finding where the center of the cursor should be in PIXELS
        position_x = int(width * nose_base.x)
        position_y = int(height * nose_base.y)

        #drawing the cursor:
          #since the image is flipped, the X moviments get flipped too, so to fix it you have to make the width minus the point...
